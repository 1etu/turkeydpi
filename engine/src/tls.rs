...
pub fn parse_client_hello(data: &[u8]) -> Option<ClientHelloInfo> {
    ...
    let _handshake_length =
        u32::from_be_bytes([0, data[pos], data[pos + 1], data[pos + 2]]) as usize;
    pos += 3;
            ...
            if pos + 5 <= data.len() && pos + ext_len <= data.len() {
                ...
                if name_type == SNI_HOST_NAME {
                    ...
                    if name_offset + name_len <= data.len() {
                        if let Ok(hostname) =
                            std::str::from_utf8(&data[name_offset..name_offset + name_len])
                        {
...
pub fn fragment_at_offsets(data: &[u8], offsets: &[usize]) -> Vec<BytesMut> {
    ...
    let mut sorted_offsets: Vec<usize> = offsets
        .iter()
...
mod tests {
    ...
    fn sample_client_hello() -> Vec<u8> {
        vec![
            0x16, 0x03, 0x01, 0x00, 0xf1, 0x01, 0x00, 0x00, 0xed, 0x03, 0x03, 0x01, 0x02, 0x03,
            0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11,
            0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f,
            0x20, 0x00, 0x00, 0x04, 0x13, 0x01, 0x13, 0x02, 0x01, 0x00, 0x00, 0x1e, 0x00, 0x00,
            0x00, 0x10, 0x00, 0x0e, 0x00, 0x00, 0x0b, 0x64, 0x69, 0x73, 0x63, 0x6f, 0x72, 0x64,
            0x2e, 0x63, 0x6f, 0x6d, 0x00, 0x15, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ]
    }
