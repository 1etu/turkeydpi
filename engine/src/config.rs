    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.transforms.jitter.max_ms > self.limits.max_jitter_ms {
            return Err(EngineError::validation(
                ...
                format!("exceeds safety limit of {}ms", self.limits.max_jitter_ms),
            ));
        }
...
impl Rule {
    pub fn validate(&self) -> Result<()> {
        ...
        if self.transforms.is_empty() {
            return Err(EngineError::validation(
                "transforms",
                "must specify at least one transform",
            ));
        }
        ...
        if let Some(ref ips) = self.dst_ip {
            for ip in ips {
                ip.parse::<IpNet>()
                    ...
                    .map_err(|_| {
                        EngineError::validation("dst_ip", format!("invalid IP/CIDR: {}", ip))
                    })?;
            }
        }
        ...
        if let Some(ref ips) = self.src_ip {
            for ip in ips {
                ip.parse::<IpNet>()
                    .or_else(|_| ip.parse::<IpAddr>().map(IpNet::from))
                    .map_err(|_| {
                        EngineError::validation("src_ip", format!("invalid IP/CIDR: {}", ip))
                    })?;
            }
        }
